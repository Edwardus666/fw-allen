...
static uint32_t printPointCsv(char *s, uint32_t dataLength, uint32_t timeStamp, int32_t value) {
    return snprintf(s, dataLength, "%lu;%ld\n", timeStamp,             value);
}
static uint32_t printPointCsvUnsigned(char *s, uint32_t dataLength, uint32_t timeStamp, int32_t value) {
    return snprintf(s, dataLength, "%lu;%lu\n", timeStamp, (uint32_t)  value);
}
static uint32_t printPointBinary(char *s, uint32_t dataLength, uint32_t timeStamp, int32_t value) {
    if(dataLength < 8) return 0;
    CO_memcpySwap4(s, &timeStamp);
    CO_memcpySwap4(s+4, &value);
    return 8;
}
static uint32_t printPointSvgStart(char *s, uint32_t dataLength, uint32_t timeStamp, int32_t value) {
    return snprintf(s, dataLength, "M%lu,%ld", timeStamp,             value);
}
static uint32_t printPointSvgStartUnsigned(char *s, uint32_t dataLength, uint32_t timeStamp, int32_t value) {
    return snprintf(s, dataLength, "M%lu,%lu", timeStamp, (uint32_t)  value);
}
static uint32_t printPointSvg(char *s, uint32_t dataLength, uint32_t timeStamp, int32_t value) {
    return snprintf(s, dataLength, "H%luV%ld", timeStamp,             value);
}
static uint32_t printPointSvgUnsigned(char *s, uint32_t dataLength, uint32_t timeStamp, int32_t value) {
    return snprintf(s, dataLength, "H%luV%lu", timeStamp, (uint32_t)  value);
}
...
static CO_SDO_abortCode_t CO_ODF_trace(CO_ODF_arg_t *ODF_arg) {
    ...
    case 1:     /* dataLength */
        if(ODF_arg->reading) {
            uint32_t *value = (uint32_t*) ODF_arg->data;
            uint32_t dataLength = trace->bufferSize;
    ...
    case 5:     /* plot */
        if(ODF_arg->reading) {
             ...
             * to print, than is the dataLength of SDO buffer, then this function
