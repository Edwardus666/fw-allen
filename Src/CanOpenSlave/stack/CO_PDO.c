...
#ifdef CFG_EX_FRAME	//Allen modified
#include "UnitOd.h"
#endif
...
#ifdef CFG_EX_FRAME
			RPDO->ident[1] = msg->ident;	//Allen modified;
#endif
		}
        else {
...
#ifdef CFG_EX_FRAME
			RPDO->ident[0] = msg->ident;	//Allen modified;
#endif
		}
    }
}
...
static void CO_TPDOconfigCom(CO_TPDO_t* TPDO, uint32_t COB_IDUsedByTPDO, uint8_t syncFlag){
    ...
    /* is TPDO used? */
    if((COB_IDUsedByTPDO & 0xBFFFF800L) == 0 && ID){	//Allen修改，移除条件TPDO->dataLength，dataLength=0也要正确初始化CANtxBuff
        /* is used default COB-ID? */
    ...
    if(TPDO->CANtxBuff == 0 || TPDO->dataLength == 0){	//Allen修改，增加条件TPDO->dataLength，
        TPDO->valid = false;
    }
}
 ...
 * @return 0 on success, otherwise SDO abort code.
 */
uint32_t CO_RPDOconfigMap(CO_RPDO_t* RPDO, uint8_t noOfMappedObjects){
    int16_t i;
    uint8_t length = 0;
    uint32_t ret = 0;
 ...
 * @return 0 on success, otherwise SDO abort code.
 */
uint32_t CO_TPDOconfigMap(CO_TPDO_t* TPDO, uint8_t noOfMappedObjects){
    int16_t i;
    uint8_t length = 0;
    uint32_t ret = 0;
    ...
    for(i=noOfMappedObjects; i>0; i--){
        ...
        if(ret){
            length = 0;
			PFL_ERROR("TPDO mappint error, map=%X", map);			
 ...
 * For more information see file CO_SDO.h.
 */
CO_SDO_abortCode_t CO_ODF_RPDOmap(CO_ODF_arg_t *ODF_arg){
    CO_RPDO_t *RPDO;
...
#ifdef CFG_EX_FRAME
	RPDO->idx_PDOMapPar = idx_RPDOMapPar;
	RPDO->pdoNo = idx_RPDOMapPar - OD_H1600_RXPDO_1_MAPPING;
#endif
        ...
        uint16_t                CANdevTxIdx)
{
...
#ifdef CFG_EX_FRAME
	TPDO->idx_MapPar = idx_TPDOMapPar;
	TPDO->pdoNo = idx_TPDOMapPar - OD_H1A00_TXPDO_1_MAPPING;
#endif
	...
	if(!TPDO->valid) TPDO->sendRequest = 0;
    return CO_ERROR_NO;
}
...
uint8_t CO_TPDOisCOS(CO_TPDO_t *TPDO){
	...
	//static uint32 printfCount1 = 20;
	//for(int i = 0; i < TPDO->dataLength && printfCount1 > 0; i++)
	//{
	//	printfCount1--;
	//	Printf("Data[%d]=0x%x, 0x%x\n", i, *TPDO->mapPointer[0], TPDO->CANtxBuff->data[i]);
	//	//if(*TPDO->mapPointer[0] == TPDO->CANtxBuff->data[i])
	//}
	//uint8_t isMatch = (*(--pPDOdataByte) == **(--ppODdataByte));
	
    switch(TPDO->dataLength){
		case 8: if(*(--pPDOdataByte) != **(--ppODdataByte) && (TPDO->sendIfCOSFlags&0x80)) { isTrue = 1; break; };
        case 7: if(*(--pPDOdataByte) != **(--ppODdataByte) && (TPDO->sendIfCOSFlags&0x40)) { isTrue = 1; break; };
        case 6: if(*(--pPDOdataByte) != **(--ppODdataByte) && (TPDO->sendIfCOSFlags&0x20)) { isTrue = 1; break; };
        case 5: if(*(--pPDOdataByte) != **(--ppODdataByte) && (TPDO->sendIfCOSFlags&0x10)) { isTrue = 1; break; };
        case 4: if(*(--pPDOdataByte) != **(--ppODdataByte) && (TPDO->sendIfCOSFlags&0x08)) { isTrue = 1; break; };
        case 3: if(*(--pPDOdataByte) != **(--ppODdataByte) && (TPDO->sendIfCOSFlags&0x04)) { isTrue = 1; break; };
        case 2: if(*(--pPDOdataByte) != **(--ppODdataByte) && (TPDO->sendIfCOSFlags&0x02)) { isTrue = 1; break; };
        case 1: if(*(--pPDOdataByte) != **(--ppODdataByte) && (TPDO->sendIfCOSFlags&0x01)) { isTrue = 1; break; };
    }
	//if (isTrue  && printfCount1 > 0)
	//{
	//	printfCount1--;
	//	PFL(DL_TPDO, "txBuf:");
	//	DUMP_BYTE_LEVEL(DL_TPDO, TPDO->CANtxBuff->data, TPDO->dataLength);
	//	PFL(DL_TPDO, "odBuf:");
	//	for(int i = 0; i < TPDO->dataLength; i++)
	//	{
	//		PFL(DL_TPDO, "%02X ", *TPDO->mapPointer[i]);
	//		//if(*TPDO->mapPointer[0] == TPDO->CANtxBuff->data[i])
	//	}
	//	PFL(DL_TPDO, "\n");
	//	//DUMP_BYTE_LEVEL(DL_TPDO, TPDO->mapPointer, TPDO->dataLength);
	//	//Printf("Data[%d]=0x%x, 0x%x\n", i, *TPDO->mapPointer[0], TPDO->CANtxBuff->data[i]);
	//}
    return isTrue;
}
...
int16_t CO_TPDOsend(CO_TPDO_t *TPDO){
	...
	DUMP_BYTE_EX_LEVEL(DL_TPDO, ("CAN1-TPDO[%X]:", TPDO->CANtxBuff->ident), TPDO->CANtxBuff->data, TPDO->dataLength);
...
#define RPDO_CALLS_EXTENSION /*Redefined by Kim*/
/******************************************************************************/
void CO_RPDO_process(CO_RPDO_t *RPDO, bool_t syncWas){
    ...
    else if(!RPDO->synchronous || syncWas)
    {
        ...
        while(RPDO->CANrxNew[bufNo]){
            ...
            uint8_t** ppODdataByte;
#ifdef CFG_EX_FRAME
			RPdo_SwitchOd(RPDO, RPDO->ident[bufNo] & 0x3FFFFU);
#endif
            i = RPDO->dataLength;
            ...
            if(RPDO->SDO->ODExtensions){
                /* for each mapped OD, check mapping to see if an OD extension is available, and call it if it is */
			CO_SDO_abortCode_t retCode = CO_SDO_AB_NONE;
                const uint32_t* pMap = &RPDO->RPDOMapPar->mappedObject1;
                ...
                for(i=RPDO->RPDOMapPar->numberOfMappedObjects; i>0; i--){
                    ...
                    ODF_arg.attribute = CO_OD_getAttribute(pSDO, entryNo, subIndex);
                    ODF_arg.pFlags = CO_OD_getFlagsPointer(pSDO, entryNo, subIndex);
					//ODF_arg.data = pSDO->OD[entryNo].pData;	//Allen comment
					ODF_arg.ODdataStorage = CO_OD_getDataPointer(pSDO, entryNo, subIndex);
					ODF_arg.dataLength = CO_OD_getLength(pSDO, entryNo, subIndex);
					ODF_arg.data = &pPDOdataByte[RPDO->dataLength - ODF_arg.dataLength];	//Allen modified
					retCode = ext->pODFunc(&ODF_arg);
					//
					if (retCode == CO_SDO_AB_NONE)
					{
						memcpy((void*)ODF_arg.ODdataStorage, ODF_arg.data, ODF_arg.dataLength);
					}
				}
            }
#else
...
{
	if (!TPDO->valid || TPDO->dataLength == 0) {TPDO->sendRequest = 0; return;}
