...
{
	uint16_t entryNo = 0;
	CO_OD_findEx(SDO->OD, SDO->ODSize, index, &entryNo);
	return entryNo;
}
...
#ifdef CFG_EX_FRAME	//Allen modified
	if (Null == UnitOd_SwitchOd(SDO, SDO->efid))
	{
		SDO->state = CO_SDO_ST_IDLE;
		SDO->CANrxNew = false;
		CO_SDO_abort(SDO, CO_SDO_AB_UNSUPPORTED_ACCESS);/* Client command specifier not valid or unknown. */
		return -1;
	}
#endif
...
//判定SDO是否空闲，如果否，则不能进行数据字典切换
uint8_t CO_SDO_isIdle(CO_SDO_t* sdo)
{
	return (sdo->state == CO_SDO_ST_IDLE) && (!sdo->CANrxNew);
}
