        ...
        switch (SDO_C->state){
            case SDO_STATE_UPLOAD_INITIATED:{
                ...
                if (SCS == SCS_UPLOAD_INITIATE){
                    if(SDO_C->CANrxData[0] & 0x02){
                        uint8_t dataLength;
                        /* Expedited transfer */
                        if(SDO_C->CANrxData[0] & 0x01)/* is dataLength indicated */
                            dataLength = 4 - ((SDO_C->CANrxData[0]>>2)&0x03);    /* dataLength */
                        else
                            dataLength = 4;
                        ...
                        *pDataSize = dataLength;
                        ...
                        /* copy data */
                        while(dataLength--) SDO_C->buffer[dataLength] = SDO_C->CANrxData[4+dataLength];
                        SDO_C->state = SDO_STATE_NOTDEFINED;
                        SDO_C->CANrxNew = false;
            ...
            case SDO_STATE_UPLOAD_RESPONSE:{
                if (SCS == SCS_UPLOAD_SEGMENT){
                    ...
                    /* get dataLength */
                    dataLength = 7 - ((SDO_C->CANrxData[0]>>1)&0x07);
                    /* verify length */
                    if((SDO_C->bufferOffset + dataLength) > SDO_C->bufferSize){
                        *pSDOabortCode = CO_SDO_AB_OUT_OF_MEM;    /* Out of memory */
                        SDO_C->state = SDO_STATE_ABORT;
                        break;
                    }
            ...
            case SDO_STATE_BLOCKUPLOAD_INITIATE:{
                if (SCS == SCS_UPLOAD_BLOCK){ /*  block upload initiate response */
                    ...
                    /*  check available buffer dataLength */
            ...
            case SDO_STATE_BLOCKUPLOAD_SUB_END:{
                ...
                if(SDO_C->CANrxData[0] & 0x80) {
                    /* Is data dataLength indicated and wrong? */
