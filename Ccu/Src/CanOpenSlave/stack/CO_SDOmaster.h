...
}CO_SDOclientPar_t, OD_SDOClientParameter_t;
...
typedef struct{
    ...
    uint8_t            *buffer;
    /** By download application indicates data dataLength in buffer.
    By upload application indicates buffer dataLength */
    uint32_t            bufferSize;
    ...
    uint8_t             toggle;
    /** Server threshold for switch back to segmented transfer, if data dataLength is small.
    ...
    /** Last sector number */
    uint8_t             block_seqno;
    /** Block dataLength in current transfer */
 ...
 * @param timeDifference_ms Time difference from previous function call in [milliseconds].
 * @param SDOtimeoutTime Timeout time for SDO communication in milliseconds.
 * @param pDataSize pointer to external variable, where dataLength of received
