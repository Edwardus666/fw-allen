 ...
 *                  expedited transfer; s=1 if data dataLength is indicated).
 *  - byte 1..2:    Object index.
 *  - byte 3:       Object subIndex.
 *  - byte 4..7:    Expedited data or data dataLength if segmented transfer.
 *
 ...
 *    includes information: index, maxSubIndex, #CO_SDO_OD_attributes_t, data dataLength and
 *    pointer to variable.
 *
 *
 ...
 *     write the data (maximum dataLength is specified by length) into data buffer and
...
/**
 * SDO buffer dataLength.
 *
 * Size of the internal SDO buffer.
 *
 * Size must be at least equal to dataLength of largest variable in @ref CO_SDO_objectDictionary.
 * If data type is domain, data length is not limited to SDO buffer dataLength. If
...
typedef struct{
    ...
    uint8_t             CANrxData[8];
    /** SDO data buffer of dataLength #CO_SDO_BUFFER_SIZE. */
	...
	union
	{
		struct
		{
			uint32_t efid : 18;	//单元地址
			uint32_t sfid : 11;	//标准帧ID
 ...
 * @param dataLength Number of data bytes to be copied (max 0xFFFF).
 */
void CO_memcpy(uint8_t dest[], const uint8_t src[], const uint16_t dataLength);
 ...
 * @param ODExtensions Pointer to the externally defined array of the same dataLength
 * as ODSize.
...
//Allen added
const CO_OD_entry* CO_OD_findEx(const CO_OD_entry* pOdArray, int odSize, uint16_t index, uint16* entryNo);
 ...
 *
 * @param SDO This object.
 * @param SDOBufferSize Total dataLength of the SDO buffer.
 *
 * @return 0 on success, otherwise #CO_SDO_abortCode_t.
 */
...
//Allen, 判定SDO是否空闲，如果否，则不能进行数据字典切换
uint8_t CO_SDO_isIdle(CO_SDO_t* sdo);
