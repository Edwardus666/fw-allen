...
//    CO_HBcons_monitoredNodes            = &COO_HBcons_monitoredNodes[0];
...
#ifdef CFG_EX_FRAME
	UnitOd_Init(CCU_ADDR, &CO_OD[0], CO_OD_NoOfElements, CO_SDO_ODExtensions,
		(CO_RPDOMapPar_t*)OD_RPDOMappingParameter, CO_NO_RPDO,
		(CO_TPDOMapPar_t*)OD_TPDOMappingParameter, CO_NO_TPDO,
		(CO_TPDOCommPar_t*)OD_TPDOCommunicationParameter
	);
#endif
    for (i=0; i<CO_NO_SDO_SERVER; i++)
    {
	...
	for(i=0; i<CO_NO_RPDO; i++){
        ...
        err = CO_RPDO_init(
               ...
               &CO->NMT->operatingState,
                nodeId,
				RPDO_CAN_ID[i],	//((i<4) ? (CO_CAN_ID_RPDO_1+i*0x100) : 0),
                0,
	...
	static const uint16 TPDO_CAN_ID[] = { CO_CAN_ID_TPDO_1, CO_CAN_ID_TPDO_2, CO_CAN_ID_TPDO_3 , CO_CAN_ID_TPDO_4  , CO_CAN_ID_TPDO_5  , CO_CAN_ID_TPDO_6,
								 CO_CAN_ID_TPDO_7, CO_CAN_ID_TPDO_8, CO_CAN_ID_TPDO_9 , CO_CAN_ID_TPDO_10 , CO_CAN_ID_TPDO_11 , CO_CAN_ID_TPDO_12};
    for(i=0; i<CO_NO_TPDO; i++){
        err = CO_TPDO_init(
                CO->TPDO[i],
                CO->em,
...
Bool CO_process_node_TPDO(
	CO_t* CO,
	bool_t                  syncWas,
	uint32_t                timeDifference_us,
	MasterNode* pMasterNode)
{
	Bool isPending = False;	//是否有Pending的帧
	int16_t i;
	CO_TPDO_t* tpdo = CO->TPDO[0];

	if (pMasterNode == Null)
	{
		/* Verify PDO Change Of State and process PDOs */
		for (i = 0; i < CO_NO_TPDO && tpdo->valid; i++, tpdo = CO->TPDO[i]) {
#ifdef CFG_EX_FRAME
			TPdo_SwitchOd(tpdo, CCU_ADDR);
			//static uint32 printfCount = 20;
			//for (int j = 0; j < tpdo->dataLength && printfCount > 0; j++)
			//{
			//	printfCount--;
			//	Printf("mapPointer[%d]@[0x%x]=%x\n", j, tpdo->mapPointer[j], *tpdo->mapPointer[j]);
			//	//tpdo->mapPointer[j] = tpdo->mapPointer[j];
			//}
#endif

			if (!tpdo->sendRequest)
				tpdo->sendRequest = CO_TPDOisCOS(tpdo);
			CO_TPDO_process(tpdo, CO->SYNC, syncWas, timeDifference_us);

			isPending |= tpdo->sendRequest;
		}
	}
	else
	{
		tpdo = CO->TPDO[0];
		for (int j = 0; j < CO_NO_TPDO; j++, tpdo = CO->TPDO[j]) {
#ifdef CFG_EX_FRAME
			TPdo_SwitchOd(tpdo, pMasterNode->addr);
#endif

			if (!tpdo->sendRequest) tpdo->sendRequest = CO_TPDOisCOS(tpdo);
        ...
        uint32_t                timeDifference_us)
{
 //   int16_t i;
	//CO_TPDO_t* tpdo;
	static int nodeInd = 0;
	static MasterNode* pMasterNode = Null;

#ifdef CFG_EX_FRAME
	//SDO 忙，不能处理PDO，PDO会引起数据字典切换，影响SDO数据处理。
	if (!CO_SDO_isIdle(CO->SDO[0]) || can_mailbox_is_full(CAN1))
	{
		return;
	}

	//每次处理一个节点的TPDO，如果因为发送时间太快而没有发送成功的帧，则必须留待下次重新发送，全部发送完毕才能切换到下一个节点。
	if (CO_process_node_TPDO(CO, syncWas, timeDifference_us, pMasterNode)) 
		return;
	...
	if(g_pMaster)
		pMasterNode = g_pMaster->nodes[nodeInd];
#endif
//	/* Verify PDO Change Of State and process PDOs */
//    for(i=0; i<CO_NO_TPDO; i++){
//		tpdo = CO->TPDO[i];
...
//        if(!tpdo->sendRequest) 
//			tpdo->sendRequest = CO_TPDOisCOS(tpdo);
//		isSent = tpdo->sendRequest;
//        CO_TPDO_process(tpdo, CO->SYNC, syncWas, timeDifference_us);
//		if (isSent != tpdo->sendRequest) return;
//    }
//
//#ifdef CFG_EX_FRAME
//	//切换所有节点的数据字典
//	MasterNode* pMasterNode = g_pMaster->nodes[0];
//	for (int i = 0; i < MAX_LIST_COUNT && pMasterNode && g_pMaster; i++, pMasterNode = g_pMaster->nodes[i])
//	{
//		for (int j = 0; j < CO_NO_TPDO; j++) {
//			tpdo = CO->TPDO[j];
//
//			TPdo_SwitchOd(tpdo, pMasterNode->addr);
//
