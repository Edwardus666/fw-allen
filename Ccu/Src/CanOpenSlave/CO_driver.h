 ...
 * @{
 */
    #define CO_LOCK_CAN_SEND()	 nvic_irq_disable(CAN1_TX_IRQn)	/**{__disable_irq();} < Lock critical section in CO_CANsend() */
    #define CO_UNLOCK_CAN_SEND() nvic_irq_enable(CAN1_TX_IRQn, 0, 0)	/**{__enable_irq();}  < Unlock critical section in CO_CANsend() */

    #define CO_LOCK_EMCY()      /**{__disable_irq();} < Lock critical section in CO_errorReport() or CO_errorReset() */
    #define CO_UNLOCK_EMCY()    /**{__enable_irq();}  < Unlock critical section in CO_errorReport() or CO_errorReset() */

    #define CO_LOCK_OD()        /**{__disable_irq();} < Lock critical section when accessing Object Dictionary */
    #define CO_UNLOCK_OD()      /**{__enable_irq();}  < Unock critical section when accessing Object Dictionary */
/** @} */
...
void CAN1_Start();
void DumpFilter();
void CANrxMsg_Dump(const CO_CANrxMsg_t* pMsg, uint32_t debugLevel, const char* tag);
